...
function useDragOpacity({ id, index,ref }: Omit<Card, 'text' | 'moveCard'> & {
...
function useDropOpacity({ id, index, ref, moveCard }: Omit<Card, 'text'> & {
    ref: React.MutableRefObject<HTMLDivElement|null>
}) {
    const [, drop] = useDrop<Omit<Card, 'text'>, Omit<Card, 'text'>, Omit<Card, 'text'>>({
        accept: CARD,
        hover(item, monitor) {
            //被拖动卡片的索引
